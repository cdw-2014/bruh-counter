...
module.exports = {
	...
	execute(client, message, args) {
		...
		else if (content.includes(CUSTOM_WORD)) countType = CUSTOM_WORD;
		else return;

		// DETERMINE EVENT TYPE
		if (content.includes(`${countType} count`)) eventType = 'getCount';
		else if (content.includes(`${countType} log`)) eventType = 'getSpreadsheet';
		else if (countType === CUSTOM_WORD && content.includes(`${CUSTOM_WORD_TRIGGER} ${CUSTOM_WORD}`))
			eventType = 'incrementCount';
		else if (countType === 'bruh' && !content.includes('bruhsound')) eventType = 'incrementCount';
		else return;
